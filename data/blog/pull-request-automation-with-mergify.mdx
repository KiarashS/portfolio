...
In this article, we'll be taking a look at [Mergify](https://mergify.com/?utm_source=article&utm_medium=dev.to&utm_campaign=KiarashS) which is a pull request automation service, triggering actions when a pull request matches defined criteria.
...
_Learn more about merge queues in detail [here](https://blog.mergify.com/what-is-a-merge-queue/?utm_source=article&utm_medium=dev.to&utm_campaign=KiarashS)_
...
_Note: Make sure to checkout full reference [here](https://docs.mergify.com/configuration/?utm_source=article&utm_medium=dev.to&utm_campaign=KiarashS)._

```yaml
pull_request_rules:
  - name: assign PRs with Txt files modified to KiarashS
    conditions:
      - files~=\.txt$
    actions:
      assign:
