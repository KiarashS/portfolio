...
In general, **if two domains are different**, then they may have different feature spaces <u>or</u> different marginal distributions.
...
2. An objective predictive function $f(.)$ (denoted by $\mathcal{T}= \{\mathcal{Y}, f(.)\}$); $f(x)$ can be written as $P(y|x)$ that shows a label based on different samples.
